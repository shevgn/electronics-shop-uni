...
export default function Dashboard<T extends Record<string, any>>({
  ...
  return (
    ...
    >
      ...
      <motion.div className="flex h-full flex-col space-y-2 overflow-hidden">
        <motion.div
          onClick={() => setIsTablePanelOpen(true)}
          className={`flex flex-row items-center justify-center space-x-2 overflow-hidden rounded-xl border-2 border-gray-200 bg-white p-1 transition-all ${isTablePanelOpen ? "h-full cursor-default" : "h-1/6 cursor-pointer hover:bg-gray-100"}`}
        >
          {!isTablePanelOpen && (
          ...
          <AnimatePresence>
            {isTablePanelOpen && (
              <div className="h-full overflow-x-hidden overflow-y-scroll border border-gray-200">
                {!isFetching ? (
                  data.length > 0 ? (
                    <Table<T>
                      data={data}
        ...
        >
          {!isTablePanelOpen && fields ? (
                  ...
                  >
                    ...
                    {field.type === "select-multiple" ? (
                      <select
                        ...
                        multiple
                        size={3}
                        className="rounded-md border border-black bg-white p-1 text-black outline-none"
                      >
            ...
            <p className="w-full text-center font-bold">No options provided</p>
          ) : (
            <p className="w-full text-center font-bold">Click to Open</p>
          )}
        </motion.div>
